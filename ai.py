...
SYSTEM_PROMPT = (
    ...
    "open. For technology, focus on AI developments with market relevance. Respond with STRICT JSON only "
...
def _make_brief_impl(sections: list, markets: dict | None, site_cfg: dict, now_utc: datetime) -> dict:
    ...
    if not api_key or not models:
        ...
        return fallback_brief(sections, markets, now_utc)
    text, model = _try_provider(provider, models, api_key, SYSTEM_PROMPT,
                                 _build_user_prompt(sections, markets, site_cfg, now_utc),
    ...
    log(f"brief generated by {provider}/{model}")
    return brief
...
def fallback_brief(sections: list, markets: dict | None, now_utc: datetime) -> dict:
    ...
    try:
        ...
        return {"headline": "Markets brief unavailable", "summary": "", "mood": None,
                ...
                "watchlist": [], "generated_by": "fallback", "model": None}
